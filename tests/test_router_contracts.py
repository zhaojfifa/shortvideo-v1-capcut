...
def test_pack_route_unique_in_primary_app() -> None:
    ...
    assert _count_routes(primary_app, "/v1/parse", "POST") == 1
    assert _count_routes(primary_app, "/v1/subtitles", "POST") == 1
    assert _count_routes(primary_app, "/v1/dub", "POST") == 1
    assert _count_routes(primary_app, "/v1/pack", "POST") == 1
    ...
    assert _count_routes(primary_app, "/api/tasks/{task_id}/dub", "POST") == 1
...
def test_pack_route_unique_in_legacy_app() -> None:
    ...
    assert _count_routes(legacy_app, "/api/tasks/{task_id}/dub", "POST") == 1
