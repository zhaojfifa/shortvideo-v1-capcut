...
- Pipeline Lab (`/ui`): trigger via `POST /v1/parse|subtitles|dub|pack`; download via `GET /v1/tasks/{task_id}/...`.

## Manual verification (Render)
- Task Workbench: parse/subtitles/dub/pack/scenes buttons call `POST /api/tasks/{task_id}/...` (Network tab).
- Pipeline Lab: parse/subtitles/dub/pack buttons call `POST /v1/...` (Network tab).
...
- Pipeline Lab triggers only `/v1/...` (no `/api/tasks` triggers).
